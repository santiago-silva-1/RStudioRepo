...
#IrishFamine7=data.frame()
for (page_result in seq(from =7754 , to= 8400, by=1)) {
...
IrishFamine7=rbind(IrishFamine7 , data.frame(LastName=LastName,FirstName=FirstName , Age= Age, NativeCountryCode= NativeCtryCode,
                                       Destination= Destination, PassengerPortOfEmbarkationCode= PortofEmbark, 
                                       ManifestIdentificationNumber=ManifestNum , 
                                       PassengerArrivalDate=ArrivalDate ,stringsAsFactors = FALSE))
print(paste("Page:", page_result, (dim(IrishFamine7))))
}
dim(IrishFamine7)
tail(IrishFamine7)
...
openxlsx::write.xlsx(IrishFamine7, file = "PeopleWhoMigratedpart7.xlsx")
